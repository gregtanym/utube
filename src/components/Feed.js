...
const Feed = () => {
    const {videos, setVideos, fetchAllVideos, fetchFilteredVideos, sidebarStatus, setSidebarStatus, toggleSidebar, setInitialSidebarState} = useGlobalContext()
                ...
                {videos.map((video) => {
                    ...
                    return(
                        <div className='feed-video-card' key={video.id}>
                            <Link to={`/${video.id}`} style={{textDecoration: 'none', width: '100%', color: 'white', position:'relative'}} onClick={onClick}>
